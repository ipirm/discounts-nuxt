<template>
  <Clink :to="{name:'discount-slug',params:{slug: post.slug ? post.slug[$i18n.locale] : '/'}}" class="overlay-card">
    ...
    <div v-else-if="post.color !== '-' && parseInt(post.ended) === 0" class="overlay-card-item"
         :style="{ backgroundColor: post.color}"><span>{{post.color_text}}</span></div>
    <div v-else-if="post.color === '-' && parseInt(post.ended) === 0" class="overlay-card-item"
         :style="{ backgroundColor: 'rgb(0, 255, 0)'}"><span>{{post.color_text}}</span></div>
    <img :alt="post.title[$i18n.locale]" :src="post.image_url"/>
